...
void kthSmallest_(TreeNode* root, multiset<int>& mySet) {
    if(root == nullptr) {
        return;
    }
    ...
    kthSmallest_(root->left, mySet);
    kthSmallest_(root->right, mySet);
}

int kthSmallest(TreeNode* root, int k) {
    multiset<int> mySet;
    kthSmallest_(root, mySet);
    auto it = mySet.begin();

    //Kth smallest value would be at k - 1 position
    advance(it, k - 1);
    return *it;
}
