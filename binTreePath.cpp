...
void binaryTreePaths_helper(TreeNode* root, string path, vector<string>& result) {
    if(root == nullptr) {
        return;
    }
    ...
    binaryTreePaths_helper(root->left, path, result);
    binaryTreePaths_helper(root->right, path, result);
}
